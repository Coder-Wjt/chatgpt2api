...
单请求最多 150 MiB，上传总时间最多 60 秒。每进程的活动写请求按入口独立计数：

- 聊天/搜索共 250 槽：POST `/v1/chat/completions`、`/v1/responses`、`/v1/messages`、`/v1/search`。聊天或 Responses 内触发的生图仍属于该入口组。
- 图片共 8 槽：POST `/v1/images/generations`、`/v1/images/edits` 及 `/api/image-tasks/` 下的 POST 请求。
- 其他写接口共 8 槽，包括管理操作和 PPT/PSD 任务提交。

活动请求槽保持到响应结束，SSE 发送响应头后仍占槽，各组满额独立返回 429。另有全局 8 路正文处理容量：正文不超过 1 MiB 时，上传完成即释放该容量；超过 1 MiB 时保持到响应结束，限制大正文同时解析及处理的数量。慢上传、超时、断连和异常均释放相应容量。GET/HEAD 查询不占这些槽位。
这些是应用入口容量，不代表已验证的上游吞吐；同步线程、账号和出口限制仍生效。共用网关 IP 的部署还须单独调整 Nginx 示例的每 IP 20 连接限制，否则会先在反向代理处被拒绝。
...
公开原图 `/images/` 与缩略图 `/image-thumbnails/` 的索引查询、WebDAV 读取及图片处理共用独立的 4 个工作线程，不占用 API 同步工作线程池，也不在事件循环中执行。处理容量满时立即返回 429 和 `Retry-After: 1`，不积累等待队列；请求取消后，已经开始的同步工作仍占用槽位直到实际结束。该限制覆盖响应准备阶段，不限制返回后的文件传输连接数；公网连接和速率限制仍由反向代理承担。
